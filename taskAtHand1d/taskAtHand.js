...
{
	var version = "v1.4dRyanK", appStorage = new AppStorage("taskAtHand");
	...
	function saveUndo(){
		var undo = []; //1d
		undo = appStorage.getValue('taskList'); //1d
		appStorage.setValue('undoList', undo); //1d
	}
	...
	$('#undoButton').click(function(){ undoTaskList();}); //1d
	function undoTaskList(){ //1d
		$('#task-list').empty();
		
		var tasks = appStorage.getValue('undoList');
		if(tasks){
			for (var i in tasks){
				addTaskElement(tasks[i]);
			}
